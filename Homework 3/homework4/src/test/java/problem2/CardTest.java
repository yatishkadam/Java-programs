...
class CardTest {
    ...
    private char shortNameOfCard = 'J' ;
    private int cardRank = 2;
    ...
    @BeforeEach
    void initClassObj(){
        card = new CardAdaptor(shortNameOfCard, cardRank, cardSymbol);
    }
    ...
    void testAllCards(){
        for (char suit:standardSuits) {
            for (Map.Entry<Character, Integer> item:deckCards.entrySet()) {
                ...
                Card card = new CardAdaptor(item.getKey(),rank,suit);

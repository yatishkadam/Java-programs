...
class PinochleTest {
    ...
    private int pinochleDeckSize = 48;
    private static final  int MAXSPADESINDECK = 12;
    private static final  int MAXHEARTINDECK = 12;
    private static final  int MAXCLUBSINDECK = 12;
    private static final int MAXDIMONDINDECK = 12;

    private int maxSameSuitCards = 12;
    private static final int MAXNUMBEROFSAMERANK = 8;

    private static final int MAXNUMBEROFSAMESUITEDRANK = 2;
    ...
    void initDeck(){
        /**
         * testing the factory method for deck.
         */
        deck = DeckFactory.createDeck("Pinochle");
    ...
    @Test
    void officialSize() {
        assertEquals(deck.officialSize(), pinochleDeckSize);
    }
    ...
    void sortByRank(){
        ...
        while (actualIndex < pinochleDeckSize){
    ...
    /**
     * This is to test the sort by both method
     * thinking ->
     *              when sorted by both
     *              the suits are sorted ina particular order
     *              the cards within each suit is ordered in a particular order
     *              so we define each of this -> the list and the suit order
     *              we iterate through the list check if the order or each card is the same as we defined.
     *
     *              in cases where we have more than 1 card of the same suit and rank we check within
     *              the loop to see if the cards are in the right order
     */
    @Test
    void sortByBoth(){
        deck.sort("Both");
    ...
    void pullCard() {
        ...
        String cardNamePulled = cardPulled.getRank().getName();
        deck.pullCard();
        ...
        // Now that the card has been pulled the card should have a count of one as the card occurs twice.
        // So the hash Map value returned will be one.
        int valueToCheck = mapOfCards.get(cardNamePulled);
        assertEquals(valueToCheck, 1);


        assertEquals(deck.getCards().size(), (pinochleDeckSize -1)); // check if the deck size shrank by 1
    }

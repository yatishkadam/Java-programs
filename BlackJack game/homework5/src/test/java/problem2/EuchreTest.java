...
/**
 * this Test class is to test the Euchre deck class
 */
class EuchreTest {
    ...
    private int euchreDeckSize = 24;
    private static final  int MAXSPADESINDECK = 6;
    private static final  int MAXHEARTINDECK = 6;
    private static final  int MAXCLUBSINDECK = 6;
    private static final  int MAXDIMONDINDECK = 6;

    private int maxSameSuitCards = 6;
    private static final int MAXNUMBEROFSAMERANK = 4;
    private static final int MAXNUMBEROFSAMESUITEDRANK = 1;
    /**
     * to create a deck
     */
    @BeforeEach
    void initEuchreDeck(){
         ...
         */
        deck = DeckFactory.createDeck("Euchre");
    ...
    @Test
    void officialSize() {
        assertEquals(deck.officialSize(), euchreDeckSize);
    }
    ...
    void sortByRank(){
        ...
        while (actualIndex < euchreDeckSize){
    ...
    void sortByBoth(){
        ...
        while ( i < euchreDeckSize){
            ...
            for (int j = i ; j < i + maxSameSuitCards;j++){
                ...
                assertEquals(suitOfCard, suitToCheck);  // check the suit of the card
                assertEquals(shortName, cardOrderToCheck); // check the order of the card.
    ...
    void pullCard() {
        ...
        // Now that the card has been pulled the key when checked should be null.
        assertNull(mapOfCards.get(cardNamePulled));

        assertEquals(deck.getCards().size(), (euchreDeckSize -1)); // check if the deck size shrank by 1
    }
